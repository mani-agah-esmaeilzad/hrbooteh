...
import { ArrowLeft, Brain, BarChart3, Frown, Smile, Meh } from 'lucide-react';
...
const Results = () => {
  ...
  useEffect(() => {
    ...
    const resultsData = localStorage.getItem('independence_results');
    if (resultsData) {
      try {
        const parsedData = JSON.parse(resultsData);
        if (parsedData.final_analysis?.analysis?.questionAnalysis) {
  ...
  if (!analysisData) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center text-center p-6">
        ...
        <p className="text-gray-600 mb-6">به نظر می‌رسد شما هنوز ارزیابی را تکمیل نکرده‌اید یا نتایج به درستی ذخیره نشده است.</p>
        <Button onClick={() => router.push('/')}>بازگشت به صفحه اصلی</Button>
      </div>
    );
  }

  const { questionAnalysis } = analysisData;
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <h1 className="text-xl font-bold text-gray-900">نتایج ارزیابی استقلال</h1>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
            <h2 className="text-xl font-semibold text-gray-800 mb-6 flex items-center">
                ...
                نمودار تحلیل ابعاد شخصیتی
            </h2>
            <div style={{ width: '100%', height: 400 }}>
                <ResponsiveContainer>
                    <RadarChart cx="50%" cy="50%" outerRadius="80%" data={questionAnalysis}>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
         ...
         <div className="text-center pt-4">
            <Button size="lg" onClick={() => router.push('/independence')}>انجام مجدد ارزیابی</Button>
        </div>
      </main>
    </div>
  );
